...
// เลี้ยวล้อเดียวแบบถอยหลัง (ใหม่) ต้องตั้งความเร็วก่อนใน Setting.ino ด้วย TurnSpeedLeftBackF()/TurnSpeedRightBackF()/TurnSpeedLeftBackB()/TurnSpeedRightBackB()
...
*   'g' || 'G': หยุดแบบใช้ไจโร
*
*   หมายเหตุ: ตัวพิมพ์ใหญ่ (P Q E C D A B G) ทำงานเหมือนตัวพิมพ์เล็กทุกประการ
*   แต่จะเรียก ToFront()/ToBack() วิ่งเข้าหาเส้นด้วย PID ก่อนเริ่มเงื่อนไข (ff=ToFront, bb=ToBack)
